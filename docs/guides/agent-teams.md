...
`/agent-teams` スキルで、specを起点にした開発サイクルを実行できる。
...
★ 人がplan + ARCHITECTURE.mdをレビュー
        ↓
/agent-teams [機能名] → チーム自動編成・実装・振り返り
        ↓
★ 人が最終確認
```

★ = 人のチェックポイント。振り返りの学びはskill内の Team Workflow Rules に蓄積され、次回に自動で引き継がれる。
...
| ファイル | 役割 |
|---|---|
| `.claude/skills/agent-teams/SKILL.md` | 起動スキル（チーム編成・フェーズ） |
| `.claude/skills/agent-teams/team-rules.md` | retroから蓄積されたチーム運用ルール |
| `.claude/skills/whiteboard/` | ホワイトボードスキル（Agent Teams非依存、単独でも使える） |
